...
if(carrito.length == 0){
    ...
    const titulo = `
        <div class='cartContainer'>
            <h2 class='txtCarrito'>Cursos seleccionados:</h2>
        </div>`;
    ...
    for (let i = 0; i < carrito.length; i++) {
        ...
        const {id, titulo, img, precio, cantidad} = element
        const cart = `
            <tr id=${id}>
            <th><img class='trash' src='../multimedia/imagenes/trash.png' alt='foto de borrar'></th>    
            <th class='detallesTabla'><img class='imgProdCart' src=${img} alt='foto del producto'><span class='nombreProd'>${titulo}</span></th>
                <th>${cantidad}</th>
